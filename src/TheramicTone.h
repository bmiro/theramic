...
#define NOTE_DIST 70 /* Distance for each note */


public:

  void TheramicTone::beat();
  void TheramicTone::mute();
  int TheramicTone::play(int note);
  int TheramicTone::play(int note, int octave);
  void TheraimcTone::stop();
  void TheramicTone::wait();

private:
/* 7 octaves of 12 notes: DO       DO#       RE       RE#       MI       FA       FA#      SOL       SOL#      LA       LA#       SI */
  int _octaves[][] = { {NOTE_C1, NOTE_CS1, NOTE_D1, NOTE_DS1, NOTE_E1, NOTE_F1, NOTE_FS1, NOTE_G1, NOTE_GS1, NOTE_A1, NOTE_AS1, NOTE_B1},
